...
def test_tp53_main():
    ...
    result = pt.main(opts)
    assert result.ix[0, 'deleterious p-value'] < 0.001, 'TP53 should have a very low p-value ({0}>.001)'.format(result[0][2])
...
def test_100genes_main():
    ...
    # single nucleotide context
    result = pt.main(opts)
    num_del_sig = np.sum(result['deleterious BH q-value'] < .1)
